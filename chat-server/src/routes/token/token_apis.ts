...
tokenRoute.post(
  ...
  wrapperFunction({
    ...
    businessLogic: async (req: Request, res: Response, next?: NextFunction) => {
      ...
      return await tokenSetFromRefreshToken(refreshToken);
    },
  })
);


let tokenSetFromRefreshToken = async function (refreshToken: string): Promise<AuthToken> {
...
export { tokenSetFromRefreshToken };
